...
class Qwen2_5_VLDecoderLayerDynamic(nn.Module):
    ...
    def __init__(self, config, layer_idx: int):
        ...
        self.top_k = 32
        self.cosine_schedule = self.cosine(start=32, end=8, total=config.num_hidden_layers)

    @staticmethod
    def cosine(start, end, total):

        idxs = np.linspace(0, 1, total)  # 归一化索引 [0, 1]
    ...
    def _inner_thinking_step(
        ...
        router_weights = F.softmax(token_scores, dim=-1)   # [B, S] 
        ...
        causal = torch.triu(torch.full((k, k), min_dtype, device=device, dtype=kept_hidden.dtype), diagonal=1)
        kept_attention_mask = causal[None, None, :, :].expand(B, 1, k, k)

        # ----- 第二次迭代：top-k token 彼此交互 -----
