...
class Qwen2_5_VLPreTrainedModel(PreTrainedModel):
    ...
    _no_split_modules = ["Qwen2_5_VLDecoderLayer", "Qwen2_5_VLVisionBlock", "Qwen2_5_VLDecoderLayerDynamic"]
...
class Qwen2_5_VLDecoderLayerDynamic(nn.Module):
    """
    ...
    def __init__(self, config, layer_idx: int):
        ...
        self.end = 16
        self.total_layer = 36
        self.layer_number = self.cosine(self.start, self.end, self.total_layer)

    def cosine(self, start, end, total):
        idxs = np.linspace(0, 1, total)
        values = (start - end) / 2 * np.cos(idxs * np.pi) + (start + end) / 2
        return np.maximum(np.round(values), end).astype(int)
    ...
    def forward(
        ...
        **kwargs,
        ...
        max_top_k_length = 32 # self.layer_number[self.layer_idx] 
        ...
        kept_tokens = torch.split(hidden_states[kept_tokens_mask], kept_tokens_lengths.tolist()) # hidden_states[kept_tokens_mask] 
        ...
        hidden_states_step1, _, _ = self._forward_block(
        ...
        for i in range(B):
            indices = router_indices[i][:kept_tokens_lengths[i]] 
...
class Qwen2_5_VLModel(Qwen2_5_VLPreTrainedModel):
    def __init__(self, config: Qwen2_5_VLConfig):
        ...
        self.layers = nn.ModuleList([
            Qwen2_5_VLDecoderLayerDynamic(config, layer_idx) if layer_idx % 2 == 1 else Qwen2_5_VLDecoderLayer(config, layer_idx)
