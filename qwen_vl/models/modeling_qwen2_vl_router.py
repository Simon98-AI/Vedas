...
def repeat_kv(hidden_states: torch.Tensor, n_rep: int) -> torch.Tensor:
    ...
    if n_rep == 1:
        return hidden_states
    # q_value_heads = num_key_value_heads * n_rep
...
class Qwen2VLAttention(nn.Module):
    ...
    def __init__(self, config: Qwen2VLConfig, layer_idx: Optional[int] = None):
        ...
        self.num_key_value_groups = self.num_heads // self.num_key_value_heads # 28 / 7
        self.is_causal = True
    ...
    def forward(
        ...
        key_states = repeat_kv(key_states, self.num_key_value_groups) # 共享kv_cache
...
class Qwen2VLDecoderLayer(nn.Module):
    def __init__(self, config: Qwen2VLConfig, layer_idx: int):
        ...
        self.self_attn = Qwen2VLAttention(config, layer_idx) # eager
    ...
    def _prepare_4d_causal_attention_mask_with_cache_position(
        ...
        if attention_mask is not None and attention_mask.dim() == 4:
            ...
            if attention_mask is not None:
                ...
                mask_length = attention_mask.shape[-1] # [1, 1, 1, 1, 0, 0]
        ...
        def hook_fn(module, grad_input, grad_output):
            ...
            if grad_input and grad_input[0] is not None:
                ...
                if self.current_epoch not in self.storage:
                    self.storage[self.current_epoch] = {}
                if layer_idx not in self.storage[self.current_epoch]:
                    self.storage[self.current_epoch][layer_idx] = []
...
class AttentionGradientCollector:
    ...
    def _register_hooks(self):
        ...
        def make_hook(layer_idx: int, param_name: str):
            def hook_fn(grad: torch.Tensor):
                ...
                self.storage[self.current_epoch][layer_idx].append(stats)
            return hook_fn
    ...
    def end_epoch(self):
        ...
        for layer_idx in self.storage[self.current_epoch]:
            batch_stats_list = self.storage[self.current_epoch][layer_idx]
...
class Qwen2VLPreTrainedModel(PreTrainedModel):
    ...
    _no_split_modules = ["Qwen2VLDecoderLayer", "Qwen2VLVisionBlock", "Qwen2VLDecoderLayerDynamic"]
...
class Qwen2VLModel(Qwen2VLPreTrainedModel):
    def __init__(self, config: Qwen2VLConfig):
        ...
        #     Qwen2VLDecoderLayerDynamic(config, layer_idx) if layer_idx % 2 == 1 else Qwen2VLDecoderLayer(config, layer_idx)
        #     for layer_idx in range(config.num_hidden_layers)
        # ])
        self.layers = nn.ModuleList([Qwen2VLDecoderLayer(config, layer_idx) for layer_idx in range(config.num_hidden_layers)])
        self.qkv_collector = AttentionGradientCollector(self)
    ...
    def _setup_gradient_tracking(self):
        """初始化时自动为所有 Decoder Layer 注册钩子"""
        for idx, layer in enumerate(self.layers):
            self.gradient_collector.register_layer(layer, idx)

    def set_tracking_epoch(self, epoch: int):
        self.gradient_collector.start_epoch(epoch)

    def finish_epoch(self):
        self.gradient_collector.end_epoch()
    ...
    def forward(
        ...
        if cache_position is None:
            past_seen_tokens = past_key_values.get_seq_length() if past_key_values is not None else 0
            cache_position = torch.arange(past_seen_tokens, past_seen_tokens + inputs_embeds.shape[1], device=inputs_embeds.device) #训练的时候会自动生成当前序列中的长度position_ids

        # the hard coded `3` is for temporal, height and width.
        if position_ids is None:
            position_ids = cache_position.view(1, 1, -1).expand(3, inputs_embeds.shape[0], -1) # positions和cache_position有一定的关系

        elif position_ids.dim() == 2:
            position_ids = position_ids[None, ...].expand(3, position_ids.shape[0], -1) ## 这种是最坏的情况了 ##
        ...
        for decoder_layer in self.layers:
            ...
            if self.gradient_checkpointing and self.training: # True
                # 传递给 decoder_layer.forward() 的实际参数
...
# @dataclass
# class Qwen2VLCausalLMOutputWithPastAndShallow(CausalLMOutputWithPast):
#     """
#     Qwen2VL causal language model outputs with shallow reasoning states.
...
#             Contains pre-computed hidden-states and key/values.
#         hidden_states (`tuple(torch.FloatTensor)`, *optional*):
#             Hidden-states of the model at each layer.
#         attentions (`tuple(torch.FloatTensor)`, *optional*):
#             Attention weights at each layer.
#         shallow_logits (`torch.FloatTensor`, *optional*):
#             Shallow reasoning logits for latent token prediction.
#         shallow_hidden_states (`tuple(torch.FloatTensor)`, *optional*):
#             Shallow hidden states for latent reasoning.
#         rope_deltas (`torch.LongTensor`, *optional*):
#             RoPE position deltas for dynamic position encoding.
#     """
...
#     hidden_states: Optional[Tuple[torch.FloatTensor]] = None
#     attentions: Optional[Tuple[torch.FloatTensor]] = None
#     shallow_logits: Optional[torch.FloatTensor] = None
#     shallow_hidden_states: Optional[Tuple[torch.FloatTensor]] = None
#     rope_deltas: Optional[torch.LongTensor] = None
